...
userProfileIconCheck()
{
    ...
    Global UserDir
    if(!FileExist(UserDir . "\awesomebar\profile.bmp"))
    {
        Loop Files, UserDir . "\AppData\Roaming\Microsoft\Windows\AccountPictures\*" 
        {
            ...
            FileDelete "*-448.bmp"
            FileMove "*-96.bmp", UserDir . "\awesomebar\profile.bmp",true
            break
            }
        }
    }
...
CheckForDownloadsInProgress(){
  ...
  {        
      ...
      {
          SendRainmeterCommand("[!SetVariable Downloading 1 awesomebar]")
          return
      }
      else
      {
          SendRainmeterCommand("[!SetVariable Downloading 0 awesomebar]")
      }
  }
}
...
showBackBar(targetWindow,instant := false)
{
    ...
    ; {
        iterations := 0
        PixelColorHex := PixelGetColor(300,50)
        startCount := A_TickCount
        While (PixelGetColor(300,50) = PixelColorHex && ((A_TickCount - startCount) < 200))
        {
            iterations++
        }
        TitlebarColor := PixelGetColor(300,50)
    ...
    SendRainmeterCommand("[!SetOption ImageBackground SolidColor `"" SplitRGBColor(TitlebarColor) "`" awesomebar\Modules\background][!SetVariable awvBGColor `"" SplitRGBColor(TitlebarColor) "`" awesomebar\Modules\background][!Update awesomebar\Modules\background][!Redraw awesomebar\Modules\background][!Show" fadeString " awesomebar\Modules\background][!SetVariable awvIsMaximized 1 awesomebar\Modules\background]")
}

hideBackBar(instant := false)
{
  fadeString := instant ? "" : "Fade"
  SendRainmeterCommand("[!SetVariable awvIsMaximized 0 awesomebar\Modules\background][!Hide" fadeString " awesomebar\Modules\background]")
}
